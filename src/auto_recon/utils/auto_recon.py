...
class AutoRecon:
	...
	def chad(self, tool: session.Tool):
		...
		query = f"site:*.{self.__args.domain} {(' OR ').join(f'ext:{ext}' for ext in ['txt', 'json', 'yml', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'zip', 'tar', 'rar', 'gzip', '7z'])}"
		run.single(
			cmd = [
				"chad -p -a random",
				run.set_opt(tool.base.args["total"      ], "-tr"   ),
	...
	def theharvester(self, tool: session.Tool):
		...
		run.single(
			cmd = [
				"theHarvester -b baidu,certspotter,chaos,commoncrawl,crtsh,duckduckgo,gitlab,hackertarget,otx,rapiddns,robtex,subdomaincenter,subdomainfinderc99,thc,threatcrowd,urlscan,waybackarchive,yahoo",
